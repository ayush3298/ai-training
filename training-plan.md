...
|-------|-------|---------------|
| Beyond Text — Vision, Voice & Generation | vision-in (OCR, tables, charts, multi-image); audio-in (STT, diarization); audio-out (TTS); realtime voice agents (turn-taking, latency budget); image/video generation; provenance (C2PA); multimodal eval & cost | after [Chapter 3](chapters/03-prompt-engineering.md) |
| Frameworks, the Ecosystem & MCP | raw SDKs vs orchestration frameworks (LangChain, LlamaIndex, Vercel AI SDK, Pydantic AI); build-vs-framework decision; MCP (client/server/tools/resources); hands-on MCP server | after [Chapter 5](chapters/05-agents.md) |
...
| Memory & Personalization _(or a Part in [Chapter 5](chapters/05-agents.md))_ | session vs cross-session state; what to store / what not to; summary buffers, memory store, RAG-over-history, structured profiles; personalization; user view/delete controls | after [Chapter 5](chapters/05-agents.md) |
...
5. ⬜ **The six remaining earlier grafts** (multi-tenant retrieval, tool/output safety, testing-vs-eval, concurrency/safe-render, feedback loop, resilience) — still to write.
6. ⬜ **Security, Privacy & Governance** — highest risk reduction; the *injection* + *PII-redaction* reflexes are already grafted into [Chapter 3](chapters/03-prompt-engineering.md)/[Chapter 4](chapters/04-rag.md), keep the full chapter before [Chapter 8](chapters/08-deployment.md).
7. ⬜ **Frameworks & MCP** — most time-sensitive / 2026-relevant. (Frameworks/libraries intentionally deferred for now.)
8. ⬜ **LLM Application Architecture & System Design** — ties the whole spine together; highest leverage for engineers shipping real systems.
9. ⬜ **Beyond Text** — biggest capability unlock, largest effort.
